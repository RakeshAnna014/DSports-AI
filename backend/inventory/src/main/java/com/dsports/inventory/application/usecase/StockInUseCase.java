...
import com.dsports.inventory.application.command.StockInCommand;
...
public class StockInUseCase {
    ...
    public StockInUseCase(InventoryRepository inventoryRepository) {
        this.inventoryRepository = inventoryRepository;
    }

    public Mono<InventoryResult> execute(StockInCommand command) {
        return inventoryRepository.findById(command.inventoryId())
                ...
                .flatMap(item -> {
                    item.stockIn(Quantity.from(command.quantity()));
