...
import com.dsports.inventory.application.command.ReleaseReservationCommand;
...
public class ReleaseReservationUseCase {
    ...
    public ReleaseReservationUseCase(InventoryRepository inventoryRepository) {
        this.inventoryRepository = inventoryRepository;
    }

    public Mono<InventoryResult> execute(ReleaseReservationCommand command) {
        return inventoryRepository.findById(command.inventoryId())
                ...
                .flatMap(item -> {
                    item.releaseReservation(Quantity.from(command.quantity()));
